...
export default function PurchaseInvoices() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get('/purchase-invoices', { params: { page, pageSize: 20 } });
  ...
  const loadLookups = async () => {
    try {
      ...
      setCashBoxes(Array.isArray(cd) ? cd : []);
      const pur = (Array.isArray(cd) ? cd : []).find((x: any) => x.code === 'PUR');
      if (pur) setForm((f) => ({ ...f, cashBoxId: String(pur.id) }));
    } catch { /* ignore */ }
  };
  ...
  const save = async () => {
    try {
      ...
      setSuccess('تم حفظ فاتورة المشتريات');
      setCreateOpen(false);
      load();
    } catch (e: any) {
  ...
  const remove = async (id: number) => {
    if (!confirm('حذف الفاتورة وعكس الكميات؟')) return;
    try {
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" mb={3}>
        <Typography variant="h4" fontWeight={700}>فواتير المشتريات</Typography>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={4}><TextField fullWidth label="رقم الفاتورة" value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} helperText="فارغ = تلقائي" /></Grid>
            <Grid item xs={5}><TextField fullWidth required label="المورد" value={form.supplierName} onChange={(e) => setForm({ ...form, supplierName: e.target.value })} /></Grid>
            <Grid item xs={3}><TextField fullWidth label="هاتف المورد" value={form.supplierPhone} onChange={(e) => setForm({ ...form, supplierPhone: e.target.value })} /></Grid>
            ...
            {lines.map((line, idx) => (
              <Grid item xs={12} key={idx} container spacing={1}>
                  ...
                  <TextField fullWidth select label="المادة" value={line.productId}
                    onChange={(e) => {
                      ...
                      next[idx] = {
                        ...next[idx],
                        productId: v,
                        unitCost: mat ? String(mat.costPrice ?? 0) : next[idx].unitCost,
                      };
                      setLines(next);
                    }}>
                    {materials.map((m) => (
                      <MenuItem key={m.id} value={m.id}>{m.code ? `${m.code} — ` : ''}{m.name}</MenuItem>
                    ))}
                  </TextField>
                </Grid>
                <Grid item xs={2}>
                ...
                <Grid item xs={2}>
                  <TextField fullWidth type="number" label="التكلفة" value={line.unitCost}
                    onChange={(e) => { const n = [...lines]; n[idx].unitCost = e.target.value; setLines(n); }} />
                </Grid>
                <Grid item xs={2}>
                  <TextField fullWidth label="الإجمالي" value={((Number(line.quantity) || 0) * (Number(line.unitCost) || 0)).toFixed(2)} InputProps={{ readOnly: true }} />
                </Grid>
                <Grid item xs={1}>
                  <Button color="error" disabled={lines.length <= 1} onClick={() => setLines(lines.filter((_, i) => i !== idx))}>حذف</Button>
                </Grid>
              </Grid>
            ))}

            <Grid item xs={12}>
              <Button onClick={() => setLines([...lines, { productId: '', quantity: '1', unitCost: '0' }])}>+ بند</Button>
            </Grid>
            ...
            <Grid item xs={3}>
              <TextField fullWidth select label="حالة الدفع" value={form.paymentStatus} onChange={(e) => setForm({ ...form, paymentStatus: e.target.value })}>
            ...
            {form.paymentStatus === 'Partial' && (
              <Grid item xs={4}><TextField fullWidth type="number" label="المبلغ المدفوع" value={form.paidAmount} onChange={(e) => setForm({ ...form, paidAmount: e.target.value })} /></Grid>
            )}
            <Grid item xs={4}>
              <TextField fullWidth select label="الصندوق" value={form.cashBoxId} onChange={(e) => setForm({ ...form, cashBoxId: e.target.value })}>
                {cashBoxes.map((b) => <MenuItem key={b.id} value={b.id}>{b.name}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12}><TextField fullWidth label="ملاحظات" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {detail && (
            <Box>
              <Typography>المورد: {detail.supplierName}</Typography>
